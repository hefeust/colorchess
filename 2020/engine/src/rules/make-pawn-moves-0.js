...
//    const side_to_move = flags.get_pair('current-player').value
    ...
    const test_promote_row = (ref) => {
        const promote_row = get_side_param(proponent, 'promote-row')

        return ref[1] === promote_row
    }

    const test_hoe_row = (ref) => {
        const home_row = get_side_param(proponent, 'fome-row')

        return ref[1] === how_row
    }
...
//            if(dest_ref[1] === get_side_param(proponent, 'promote-row') ) {
    ...
    futures.map((subpath) => {
        ...
        if(fen_promote === '') {
            ...
            if(test_home_row(src)) {
                delta_row = get_side_param(proponent, 'delta-row')
                seq = src.sequence({ col: 0, row: delta_row })
